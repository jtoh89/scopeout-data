...
import datetime
from utils.utils import calculate_percentiles_from_list, string_to_float, string_to_int
...
def import_redfin_zipcode_historical_data(geo_level, geoid_field, prod_env):
    ...
    earliest_date = datetime.datetime(REDFIN_MIN_YEAR_ZIPCODES, 1, 1)
    ...
    with open(rootpath + file_dir, 'r') as csvfile:
        ...
        for i, row in enumerate(datareader):
            ...
            year, month, day = row_dict['period_begin'].split('-')
    ...
    for k, zip_data in zipcode_data_dict.items():
        temp_df = pd.DataFrame.from_dict(zip_data).sort_values(by='date')
        temp_dict = {
            'zipcode': k,
            'date': [],
            'median_sale_price': [],
        ...
        for i, row in temp_df.iterrows():
            ...
            if row['date'].month - 1 != prev_month:
                print("PREVIOUS MONTH MISSING?")
                sys.exit()
            else:
                prev_month = row['date'].month
            try:
                temp_dict['date'].append(INDEX_TO_MONTH[row['date'].month-1] + ' ' + str(row['date'].year))
            except Exception as e:
                print(e)

            temp_dict['median_sale_price'].append(row.median_sale_price)
            temp_dict['median_sale_price_mom'].append(row.median_sale_price_mom)
            temp_dict['median_sale_price_yoy'].append(row.median_sale_price_yoy)
            temp_dict['median_dom'].append(row.median_dom)
            temp_dict['median_dom_mom'].append(row.median_dom_mom)
            temp_dict['median_dom_yoy'].append(row.median_dom_yoy)
            temp_dict['median_ppsf'].append(row.median_ppsf)
            temp_dict['median_ppsf_mom'].append(row.median_ppsf_mom)
            temp_dict['median_ppsf_yoy'].append(row.median_ppsf_yoy)

        insert_list.append(temp_df)


    client = mongoclient.connect_to_client(prod_env=prod_env)
    dbname = 'MarketTrends'
    db = client[dbname]
    collection = db['zipcodehistorical']
    ...
    #                               dbname='MarketTrends',
    #                               collection_name='zipcodehistorical',
